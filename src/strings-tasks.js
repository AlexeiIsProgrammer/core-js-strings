...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce(
      (acc, curr) => acc + (vowels.includes(curr.toLowerCase()) ? 1 : 0),
      0
    );
}
    ...
    .map((el) => {
      ...
      if (el === el.toUpperCase()) {
        if (ALPHABET_BIG.indexOf(el) + 13 >= ALPHABET_BIG.length) {
          return ALPHABET_BIG[
            (ALPHABET_BIG.indexOf(el) + 13) % ALPHABET_BIG.length
          ];
        }
      ...
      if (ALPHABET_SMALL.indexOf(el) + 13 >= ALPHABET_SMALL.length) {
        return ALPHABET_SMALL[
          (ALPHABET_SMALL.indexOf(el) + 13) % ALPHABET_SMALL.length
        ];
      }
